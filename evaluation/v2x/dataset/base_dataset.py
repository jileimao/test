...
class DAIRV2XDataset(Dataset):
    def __init__(self, path, args, split="train", extended_range=None):
        super().__init__()
        # self.size = len(load_json(osp.join(path, "data_info.json")))
        # self.split = self.get_split(path, split)

    def get_split(self, path, split):
        split_path = osp.join(path, "cooperative/split.json")
        if osp.exists(split_path):
            split_data = load_json(split_path)
            train = [int(x) for x in split_data["train"]]
            val = [int(x) for x in split_data["val"]]
            test = [int(x) for x in split_data["test"]]

...
parser.add_argument(
    "--read_path", type=str, default="./single-vehicle-side-release", help="location of read training label dir"
)
parser.add_argument(
    "--write_path",
    type=str,
...
parser_read_path = args.read_path
parser_write_path = args.write_path

######################################## utils ######################################################
#####################################################################################################
def get_path(my_path):
    path = os.path.join(parser_read_path, my_path)
    return path
...
def get_label(label):
    ...
    return h, w, l, x, y, z, rotation_y


def set_label(label, h, w, l, x, y, z, alpha, rotation_y):
    label["3d_dimensions"]["h"] = h
    label["3d_dimensions"]["w"] = w
    label["3d_dimensions"]["l"] = l
...
############################# get alpha and rotation_y in camera coordinate #######################
####################################################################################################
...
def change_hwl(h, w, l):
    h, l, w = l, w, h
    return h, w, l


def gen_lidar2cam(path_data_info, task_type="lidar"):
    data_info = read_json(path_data_info)
    write_path = os.path.join(parser_write_path, "label", task_type)
    mkdir_p(write_path)
    ...
    for data in data_info:
        ...
        calib_lidar2cam = read_json(get_path(calib_lidar2cam_path))
        r_velo2cam, t_velo2cam = get_lidar2cam(calib_lidar2cam)
        Tr_velo_to_cam = np.hstack((r_velo2cam, t_velo2cam))

        labels_path = data["label_" + task_type + "_std_path"]
        labels = read_json(get_path(labels_path))
        for label in labels:
        ...
        write_path = os.path.join(parser_write_path, labels_path)
...
if __name__ == "__main__":
    path_data_info = get_path("data_info.json")
    gen_lidar2cam(path_data_info, task_type=args.my_label_type)

...
def label_world2v(path_cooperative_label, path_novatel2world, path_lidar2novatel):
    ...
    for i in range(len(label_world)):
        ...
        l = label_world[i]["3d_dimensions"]["l"]
...
def get_rotation(world_8_points, my_3d_point, l, w):
    x = my_3d_point[0]
    a = world_8_points[0][0]
    b = world_8_points[1][0]
    c = world_8_points[2][0]
    r_tan = ((b + c - 2 * x) * l) / ((a + b - 2 * x) * w)
...
if __name__ == "__main__":
    path_c = "/home/yuhaibao/Desktop/Baseline-code/Feature-Fusion/dataset/dair-v2x-examples/cooperative-vehicle-infrastructure"
    path_dest = "label_new"
    get_label2v(path_c, path_dest)

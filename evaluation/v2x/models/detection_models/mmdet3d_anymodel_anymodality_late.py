...
class LateFusionInf(nn.Module):
        ...
        if self.args.sensortype == "lidar":
            ...
            if not osp.exists(path) or self.args.overwrite_cache:
                ...
                pred_dict = gen_pred_dict(
                    ...
                    points,
                    result[0]["scores_3d"].numpy()[remain].tolist(),
                    result[0]["labels_3d"].numpy()[remain].tolist(),
                )
            ...
            if not osp.exists(path) or self.args.overwrite_cache:
                ...
                # Load image and annos
                # osp.join(self.path, self.get("image_path"))
                # image = frame.image()
                # print(image.size())
                ...
                # Convert to world coordinate
                box = trans(box_camera)
                box_center = trans(box_center_camera)[:, np.newaxis, :]
                arrow_ends = trans(arrow_ends_camera)[:, np.newaxis, :]
                ...
                if len(remain) >= 1:
                    ...
                    box = np.zeros((1, 8, 3))
                    box_center = np.zeros((1, 8, 3))
                    arrow_ends = np.zeros((1, 8, 3))

                if self.args.save_image:  # TODO
                    image = frame.image(data_format="array")
                else:
                    image = np.array([])
                ...
                if len(result[0]["scores_3d"].numpy()) == 0:
                    pred_dict = gen_pred_dict(
                        id,
    ...
    def forward(self, data, trans, pred_filter, prev_inf_frame_func=None):
        try:
            ...
            pred_dict, id = self.pred(data, trans, pred_filter)
        # center, size = box2info(np.array(pred_dict["boxes_3d"]))
            ...
            if prev_frame is not None:
                try:
                    pred_dict, _ = self.pred(
                        prev_frame,
                        prev_frame.transform(from_coord="Infrastructure_lidar", to_coord="World"),
                        pred_filter,
                    )
                except Exception:
                    logger.info("building model")
...
class LateFusion(BaseModel):
    ...
    def forward(self, vic_frame, filt, prev_inf_frame_func=None, *args):
        ...
        offset = np.repeat(
            np.array([[float(vic_frame.offset["delta_x"]), float(vic_frame.offset["delta_y"])]]), pred_inf.num_boxes, 0
        )
        pred_inf.move_center(offset)

...
class InfOnly(BaseModel):
    ...
    def forward(self, vic_frame, filt, offset, *args):
        self.model(
            ...
            vic_frame.transform(from_coord="Infrastructure_lidar", to_coord="World"),
            filt,
        )
        # print(vic_frame.keys())
        pred = np.array(self.pipe.receive("boxes"))
        offset_mat = np.ones_like(pred)
        offset_mat[:, :, 0] *= vic_frame.offset["delta_x"]
        offset_mat[:, :, 1] *= vic_frame.offset["delta_y"]
        offset_mat[:, :, 2] = 0
        return {
            "boxes_3d": pred + offset_mat,
...
class VehOnly(BaseModel):
    ...
    def forward(self, vic_frame, filt, *args):
        pred = self.model(
            vic_frame.vehicle_frame(), vic_frame.transform(from_coord="Vehicle_lidar", to_coord="World"), filt
        )[0]
        return {

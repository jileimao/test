...
class ReduceInfTC(nn.Module):
    def __init__(self, channel):
        ...
        self.conv1_2 = nn.Conv2d(channel//2, channel//4, kernel_size=3, stride=2, padding=1)
        self.bn1_2 = nn.BatchNorm2d(channel//4, track_running_stats=True)
        self.conv1_3 = nn.Conv2d(channel//4, channel//8, kernel_size=3, stride=2, padding=1)
        self.bn1_3 = nn.BatchNorm2d(channel//8, track_running_stats=True)
        self.conv1_4 = nn.Conv2d(channel//8, channel//16, kernel_size=3, stride=2, padding=0)
        self.bn1_4 = nn.BatchNorm2d(channel//16, track_running_stats=True)

        self.deconv2_1 = nn.ConvTranspose2d(channel//16, channel//8, kernel_size=3, stride=2, padding=0)
...
class FeatureFlowNet(SingleStage3DDetector):
    ...
    def flownet_init(self):
        ...
        pretraind_checkpoint_path=osp.abspath('../../MMDetection3D/v2x_work_dir/checkpoints/epoch_80.pth')
